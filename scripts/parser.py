...
class FeatherParser(object):
    ...
    def process(self, file, columns=None):
        ...
        while window_right <= file_data.index[-1]:
            ...
            self.data["Дисперсия флуктуаций скорости ветра"].append(pd.DataFrame(index=[t], data={"Дисперсия флуктуаций скорости ветра": [sigma2_dV]}).rename_axis("t"))
            # Дисперсия флуктуаций скорости звука
            sigma2_dCs = float(dCs.pow(2).mean())
            self.data["Дисперсия флуктуаций скорости звука"].append(pd.DataFrame(index=[t], data={"Дисперсия флуктуаций скорости звука": [sigma2_dCs]}).rename_axis("t"))
